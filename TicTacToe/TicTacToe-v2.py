...
def KomputerTurn(XPosition,OPosition,AvaliabelSpace,HorizontalWinCondition,VerticalWinCondition,DiagonalLeftSideWinCondition,DiagonalRightSideWinCondition):
    IsWin=False
    IsTie=False
    BoardWinCondition=[
    ...
    for e in range(int(len(BoardWinCondition))):
        TryToWin(OPosition,BoardWinCondition[e],SpaceFilledbyUstoWin,AvaliabelSpace,XPosition)
    ...
    if IsWin==False:
        for q in range(int(len(BoardWinCondition))):
            TryToTie(XPosition,BoardWinCondition[q],SpaceFilledbyOpponent,SpaceNotFilledbyOpponent)
...
def Main(XTurn="human",OTurn="komp"):
    ...
    for q in range ((size*size)+1):
        ...
        if WinCondition==True:
            ...
            if q%2==0:
                if XTurn=="human":
                    HumanCoiseCordinate=HumanTurn()
                else:
                    HumanCoiseCordinate=None
                FillBoard("X",HumanCoiseCordinate)
            else:
                if OTurn=="komp":
                    KomputerChoice=KomputerTurn(XPosition,OPosition,AvaliabelSpace,HorizontalWinCondition,VerticalWinCondition,DiagonalLeftSideWinCondition,DiagonalRightSideWinCondition)
                else:
                    KomputerChoice=None
                print("komputer memilih",KomputerChoice)
                FillBoard("O",KomputerChoice)
    draw()    
    return print("pemengannya",Winner)
